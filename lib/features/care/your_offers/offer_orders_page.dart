...
class OfferOrdersPage extends Page<void> {
  ...
  Route<void> createRoute(BuildContext context) =>
      OfferOrdersScreenRoute(offer: offer, page: this);
}

class OfferOrdersScreenRoute extends MaterialPageRoute<void> {
  OfferOrdersScreenRoute({
    required Offer offer,
    OfferOrdersPage? page,
  }) : super(
          settings: page,
          builder: (context) => Orders(offer: offer),
        );
}
    ...
    return Scaffold(
      ...
      body: BlocBuilder<OrderListCubit, OrderListState>(
        ...
        builder: (context, state) {
          final offerOrders =
              state.orderList.where((element) => element.offer.id == offer.id);

          if (state.loading) {
            return const Center(child: CircularProgressIndicator());
          } else if (offerOrders.isEmpty) {
            return const Center(
          ...
          return Padding(
            ...
            child: ListView.separated(
              shrinkWrap: true,
              itemCount: offerOrders.length,
              separatorBuilder: (context, index) => const SizedBox(height: 16),
              itemBuilder: (context, index) {
                final order = offerOrders.elementAt(index);
